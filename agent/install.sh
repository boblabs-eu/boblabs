...
echo ""
echo "Once the agent is running, its detected providers (Ollama, ComfyUI,"
echo "Claude CLI, …) and their models appear in the control plane's"
echo "orchestrator console within ~10 seconds."
echo ""
echo "By default they are auto-approved and immediately dispatchable. If"
echo "the control plane is started with BOB_REQUIRE_PROVIDER_APPROVAL=true"
echo "(strict mode), an admin must click 'Approve' once on each new"
echo "provider in the orchestrator console before its models become usable."
echo "See docs/AGENT.md ('Models not showing in console?') for details."
echo ""
