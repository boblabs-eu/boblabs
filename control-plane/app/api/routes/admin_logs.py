...
async def list_request_logs(
    ...
    if status_code is not None:
        where.append("status = :status_code")
...
async def request_log_metrics(
    ...
    timeseries_res = await db.execute(
        text(
            "SELECT to_timestamp("
    ...
    by_module_res = await db.execute(
        text(
            "SELECT module, COUNT(*) AS c FROM request_log "
            "WHERE timestamp >= :since GROUP BY module ORDER BY c DESC"
        ),
    ...
    recent_errors_res = await db.execute(
        text(
    ...
    return {
        ...
        "by_status": [
            {"bucket": int(r.bucket), "count": int(r.c)} for r in by_status_res.fetchall()
        ],
        "top_paths": [{"path": r.path, "count": int(r.c)} for r in top_paths_res.fetchall()],
        "top_users": [
            {"user_email": r.user_email, "count": int(r.c)} for r in top_users_res.fetchall()
        ],
