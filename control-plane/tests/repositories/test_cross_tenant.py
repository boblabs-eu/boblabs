...
async def test_lab_agent_get_all_user_sees_only_accessible_labs(
    db, lab_factory, admin_user, regular_user,
):
...
async def test_portfolio_pnl_scoped_by_lab(db, admin_user, lab_factory):
    ...
    db.add_all([
        TradingPosition(
            id=uuid.uuid4(), wallet_address="0xaa", chain="eth",
            token_address="0x1", token_symbol="ALPHA", amount=1.0,
            entry_price_usd=100.0, status="open", lab_id=lab_a.id,
        ),
        TradingPosition(
            id=uuid.uuid4(), wallet_address="0xbb", chain="eth",
            token_address="0x2", token_symbol="BETA", amount=2.0,
            entry_price_usd=50.0, status="open", lab_id=lab_b.id,
        ),
    ])
    await db.commit()
...
async def test_get_all_memories_refuses_without_share_memory_confirmation(
    ...
    with pytest.raises(PermissionError) as exc:
        await repo.get_all_memories(
            caller_lab_id=lab.id, share_memory_confirmed=False,
        )
...
async def test_get_all_memories_succeeds_with_confirmation(
    ...
    db.add_all([
        LabMemory(
            id=uuid.uuid4(), lab_id=lab1.id,
            scope="lab", key="k1", content="lab1-memory",
        ),
        LabMemory(
            id=uuid.uuid4(), lab_id=lab2.id,
            scope="lab", key="k2", content="lab2-memory",
        ),
    ])
    await db.commit()
    repo = LabMemoryRepository(db)
    rows = await repo.get_all_memories(
        caller_lab_id=lab1.id, share_memory_confirmed=True, limit=10,
    )
