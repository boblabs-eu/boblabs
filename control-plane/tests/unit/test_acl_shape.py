...
async def test_acl_with_non_string_owner_rejected(db):
    ...
    lab = Lab(id=uuid.uuid4(), name="bad-owner", acl={"owner": 12345, "editors": [], "viewers": []})
    db.add(lab)
...
async def test_acl_missing_required_keys_rejected(db):
    ...
    lab = Lab(id=uuid.uuid4(), name="missing-owner", acl={"editors": [], "viewers": []})
    db.add(lab)
    with pytest.raises((IntegrityError, DBAPIError)):
        await db.commit()
    await db.rollback()
...
async def test_acl_extra_keys_allowed(db, admin_user):
    ...
    lab = Lab(
        id=uuid.uuid4(),
        name="extra-keys",
        acl={"owner": admin_user["sub"], "editors": [], "viewers": [], "tag": "agent_instance"},
    )
    db.add(lab)
...
async def test_empty_dict_acl_denies_non_admin():
    ...
    with pytest.raises(HTTPException) as exc:
        check_permission({"sub": "anyone@x", "role": "user"}, {}, Permission.VIEW)
