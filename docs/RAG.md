...
Three RAG tools are exposed to agents (orchestrator + agents). Definitions live in [tool_rag.py](../control-plane/app/services/tools/tool_rag.py) and the auto-injection logic in [rag_service.py](../control-plane/app/services/rag_service.py) (`augment_tool_names_with_rag_access`).

### `rag_list_collections`

Lists collections accessible to the current lab.

```python
TOOLS["rag_list_collections"] = {
    "description": "List the RAG collections this lab can access, including descriptions and document counts.",
    "parameters": {},
}
```

Output includes `name`, `display_name`, `document_count`, `chunk_count`, `description`, and a `[lightrag]` tag for LightRAG collections (vector collections have no tag).

### `rag_search`

Read-side search across an accessible collection.

```python
TOOLS["rag_search"] = {
    "description": "Search an accessible RAG collection using semantic similarity. "
                   "For LightRAG collections, supports mode: local (vector), global (graph), "
                   "hybrid (both). Use rag_list_collections first if needed.",
    "parameters": {
        "query":           "(required) Natural language search query",
        "collection":      "(required) Collection name from rag_list_collections",
        "top_k":           "(optional) Result count, default 5, max 20 — vector mode only",
        "mode":            "(optional) local | global | hybrid — LightRAG only",
        "filter":          "(optional) Metadata filter object — vector mode only",
        "score_threshold": "(optional) Minimum similarity 0–1, default 0.3 — vector mode only",
    },
}
```

`top_k`, `filter`, and `score_threshold` are silently ignored for LightRAG collections (LightRAG returns one consolidated context block, see [LIGHTRAG.md](LIGHTRAG.md#search-modes)). Conversely, `mode` is ignored for vector collections.

### `rag_ingest`

Write-side ingestion of text or a workspace file into a collection. Auto-injected **only** when the lab has at least one `lab_rag_access` row with `can_write=true`.

```python
TOOLS["rag_ingest"] = {
    "description": "Ingest text content or a workspace file into a RAG collection this lab has "
                   "write access to. Use rag_list_collections to find available collections.",
    "parameters": {
        "collection":  "(required) Collection name (from rag_list_collections)",
        "filename":    "(required) Source label for the ingested document",
        "source_file": "(optional) Path to workspace file to ingest, relative to /workspace",
        "content":     "(optional) Raw text content to ingest (alternative to source_file)",
        "metadata":    "(optional) Metadata tags, e.g. {\"source\": \"youtube\", \"video_id\": \"...\"}",
    },
}
```

Exactly one of `source_file` or `content` must be provided. For vector collections the document is chunked with the collection's `default_chunk_size` / `default_chunk_overlap` / `default_splitter`; for LightRAG collections those fields are ignored and the text is handed to `LightRAG.ainsert()` which does its own chunking + entity extraction. Path traversal outside `/workspace` is rejected.

Typical agentic loop: the orchestrator searches with `rag_search`, decides what new knowledge is worth keeping, then writes it back with `rag_ingest` so the next run can find it.

### Tool Availability Logic

```python
# control-plane/app/services/rag_service.py — augment_tool_names_with_rag_access()
#
# rag_list_collections + rag_search → injected when lab has ANY lab_rag_access row
# rag_ingest                        → injected only when at least one row has can_write=true
#
# Applied in three places so the tool surface matches access at every entry point:
#   - lab_runner.py        (orchestrator + agent tool resolution at run time)
#   - lab_scheduler.py     (scheduled runs)
#   - tool_executor.py     (per-call resolution)
```

A lab with read-only RAG access sees `rag_list_collections` and `rag_search` but not `rag_ingest`. Granting write on a single collection is enough to surface `rag_ingest` — the tool itself re-checks `can_write` per call against the collection the agent named.
...
| `POST` | `/api/v1/rag/collections/{id}/documents` | Upload + ingest a document (multipart) |
| `POST` | `/api/v1/rag/collections/{id}/documents/from-url` | Ingest from a public URL (fetched + HTML-sanitized server-side) |
| `DELETE` | `/api/v1/rag/collections/{id}/documents/{doc_id}` | Remove document + its chunks |
| `POST` | `/api/v1/rag/collections/{id}/documents/{doc_id}/reingest` | Re-chunk a single document with new settings |
| `POST` | `/api/v1/rag/collections/{id}/documents/reingest-all` | Re-chunk every document in the collection (batch) |
