...
The same three tools are exposed for both vector and LightRAG collections — they branch internally on `rag_mode`. See [RAG.md §9](RAG.md#9-new-tools-for-agents) for the full reference; the LightRAG-specific behavior is summarized below.

### `rag_list_collections`

Lists all RAG collections accessible to the lab. LightRAG collections are tagged `[lightrag]` in the output line (vector collections have no tag), so agents can pick the right query strategy.
...
For LightRAG collections the `mode` parameter overrides the collection's `lightrag_search_mode` default. If omitted, the collection default is used. `top_k`, `filter`, and `score_threshold` are silently ignored for LightRAG — the service returns a single context block extracted from the knowledge graph (see [Search Output](#search-output)).
...
{
  "collection": "string (required) — collection name",
  "filename":   "string (required) — source label for the document",
  "source_file":"string — workspace path (alternative to content)",
  "content":    "string — raw text (alternative to source_file)",
  "metadata":   "object — optional tags attached to the document"
}
```

Auto-injected only when the lab has at least one collection linked with `can_write=true`. For LightRAG collections, the document is handed to `LightRAG.ainsert()` and goes through the full ingestion pipeline: internal chunking → per-chunk LLM entity/relationship extraction (via the collection's `lightrag_model_id`, routed through `LabDispatcher`) → graph + KV + vector store update under `/data/lightrag/{collection_id}/`.

Two consequences worth flagging to agents that ingest into LightRAG collections:

1. **Ingestion cost is non-trivial.** Each chunk triggers an LLM call for entity extraction, so writing back a large blob with `rag_ingest` will burn through the collection's configured model. Prefer ingesting summaries or already-distilled context rather than raw transcripts.
2. **No incremental dedup on re-ingest.** Calling `rag_ingest` twice with the same `filename` does not replace the previous version — it adds another pass of entities/relationships on top of the existing graph. Re-ingest the underlying document via the API (`/documents/{doc_id}/reingest`) if you need a clean rebuild.
