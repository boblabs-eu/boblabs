...
### Core services (in [gpu-services/](../gpu-services/))
...
### Optional add-ons (top-level `docker-compose.<name>.yml` files)

These ship as standalone compose overlays in the repo root. They are not built
or wired by default — opt in with `docker compose -f docker-compose.yml -f docker-compose.<name>.yml up -d`. Most use upstream images (vLLM, ComfyUI, Postiz),
not custom code in `gpu-services/`.

| Overlay file | Port | Model / image | VRAM | Notes |
|---|---|---|---|---|
| `docker-compose.flux1.yml` | 8188 | ComfyUI + FLUX.1-dev | ~12 GB | Image generation; reachable via the `comfyui` tool. |
| `docker-compose.sdxl.yml` | 8189 | ComfyUI + SDXL | ~10 GB | Same pattern as flux1. |
| `docker-compose.sd3.yml` | 8190 | ComfyUI + SD3 | ~10 GB | Same pattern as flux1. |
| `docker-compose.pixtral12b.yml` | 8011 | vLLM + `mistralai/Pixtral-12B` | ~24 GB | OpenAI-compatible vision LLM endpoint. |
| `docker-compose.qwen7b.yml` | 8009 | vLLM + Qwen2.5-VL-7B | ~16 GB | Lightweight Qwen vision endpoint. |
| `docker-compose.qwen72b.yml` | 8010 | vLLM + Qwen2.5-VL-72B (TP=4) | ~80 GB | Best-in-class open-source vision LLM. Requires 4× GPUs. |
| `docker-compose.postiz.yml` | 4007 | Postiz social scheduler | n/a | Not GPU. Pairs with the `postiz` tool for cross-platform posting. |

These are deliberately separate compose files so they can be co-located on
purpose-built GPU hosts without dragging the rest of the GPU stack along.
