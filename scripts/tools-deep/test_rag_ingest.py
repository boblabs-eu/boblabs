...
from _harness import require_env, make_executor, run_tool, passed, fail_or_skip, run  # noqa: E402
...
async def main():
    ...
    async with make_executor(timeout_sec=120) as (db, executor):
        ing = await run_tool(executor, TOOL, {
            "collection": collection, "filename": fname, "content": content,
        })
        if not ing["success"]:
            fail_or_skip(TOOL, f"ingest: {ing['output']}")
        # Verify retrieval.
        srch = await run_tool(executor, "rag_search", {
            "query": sentinel, "collection": collection, "top_k": 3,
        })
        if not srch["success"] or sentinel not in srch["output"]:
            fail_or_skip(TOOL, f"sentinel not found via rag_search: {srch['output']}")
        passed(TOOL, f"ingested + retrieved {sentinel} in {ing['latency_ms']}+{srch['latency_ms']}ms")
